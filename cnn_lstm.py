...
def spatial_block(input, filters, kernel, pool_size, pool_stride):
    # Build the block to extract spatial convolution

    x = tf.keras.layers.Conv3D(filters, kernel, padding = 'same')(input)
    x = tf.keras.layers.LeakyReLU()(x)
    x = tf.keras.layers.MaxPooling3D(pool_size = pool_size, strides = pool_stride, padding = 'same')(x)
    x = tf.keras.layers.Conv3D(filters, kernel, padding = 'same')(x)
    x = tf.keras.layers.LeakyReLU()(x)
    x = tf.keras.layers.MaxPooling3D(pool_size = pool_size, strides = pool_stride, padding = 'same')(x)
    return x 
...
def build_model(input):
    ...
    # Spatial Block
    x = spatial_block(img_input, 32, kernel = (3,3,3), pool_size = (1,2,2), pool_stride = (1,2,2))
    x = spatial_block(x, 64, kernel = (3,3,3), pool_size = (2,2,2), pool_stride = (2,2,2))
    x = spatial_block(x, 128, kernel = (3,3,3), pool_size = (2,2,2), pool_stride = (2,2,2))
